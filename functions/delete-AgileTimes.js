...
function addTimesheet(intent, session, callback) {
    ...
    if (projectSlot && typeof (projectSlot.value) !== 'undefined') {
        ...
        speechOutput = categorySlot && typeof (categorySlot.value) !== 'undefined' ? `Got it Sanket Patil. Adding new timesheet to ${project} project, ${category} category.` : `Got it Sanket Patil. Adding new timesheet to ${project} project.`;
...
function createNewTimeSheet(projectName, categoryName) {
    ...
    var client = require('graphql-client')({ url: 'https://api.graph.cool/simple/v1/cj7wkvnfx0hwf0145ltn32czp' });
...
/**
 * Deletes alltimesheets from agileTime.
 */
function deleteAllTimesheets(intent, session, callback) {
    const cardTitle = intent.name;
    const projectSlot = intent.slots.Project;
    const categorySlot = intent.slots.Category;
    let repromptText = null;
	...
	speechOutput = '`Got it Sanket Patil. Deleting all timesheet from agile times.`'
...
/*    if (projectSlot && typeof (projectSlot.value) !== 'undefined') {
        ...
        const category = categorySlot.value;
        sessionAttributes = createAttributes(project, category);
        speechOutput = categorySlot && typeof (categorySlot.value) !== 'undefined' ? `Got it Sanket Patil. Deleting all timesheet from agile times.` : `Got it Sanket Patil. Adding new timesheet to ${project} project.`;*/
...
function removeAllTimeSheet() {

    var client = require('graphql-client')({ url: 'https://api.graph.cool/simple/v1/cj7wkvnfx0hwf0145ltn32czp' });

    client.query(`allTimesheets {
       allTimesheets {
           id
         }
     }`, function (req, res) {
        ...
        .then(function (body) {
            console.log(body.data.allTimesheet);
        })
        .catch(function (err) {
            console.log(err.message);
        });
